...
public class TaskUpdatesController {
    ...
    @PutMapping("/{taskId}")
    public ResponseEntity<?> updateTaskUpdate(@PathVariable Long taskId, @RequestBody TaskUpdate taskUpdate) {
        taskUpdatesService.updateTaskUpdate(taskId, taskUpdate);
        return ResponseEntity.ok().build();
    }
